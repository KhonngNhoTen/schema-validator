...
import { DataType, InputSchema, SchemaOptions } from "./type";
...
export function isMapSchema(object: any): object is Record<string, Schema> {
  if (!isObject(object)) return false;
  for (const [key, value] of Object.entries(object)) {
...
export function isSchemaOption(object: any): object is SchemaOptions {
  const type = (object as SchemaOptions).type;
  if (type && isDataType(type.type)) return true;
  return false;
}
