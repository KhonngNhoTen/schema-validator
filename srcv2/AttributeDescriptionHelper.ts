...
import { AttributeDescription, BaseAttributeDescription, DataAttributeTraversal, DataType, RequiredRuleOptions, TypeRuleOptions, isDataType } from "./type";
export class AttributeDescriptionHelper {
  ...
  static attributeTraversal<T>(
    ...
    if (this.havingChild(attribute)) this.getChilds(attribute).forEach((child) => {
  ...
  static getChilds<T>(attributes: AttributeDescription<T>): { key: string, attribute: BaseAttributeDescription }[] {
    ...
    if (attributes.type === "array") return [{ key: Schema.ARRAY_KEYWORD, attribute: attributes.item as BaseAttributeDescription }];
    return []
